...
def nbs_amt_calc(ch_00, ch_dep, nbs_rt):
    ...
    # at least 1 newborn
    if ch_00 > 1:
        nbs_max_amt = ch_00 * nbs_rt[1] * 365 / 91  # assign higher rate
    elif ch_00 == 1 and ch_dep == 1:
        nbs_max_amt = nbs_rt[1] * 365 / 91  # assign higher rate
    else:
        nbs_max_amt = nbs_rt[0] * 365 / 91  # assign lower rate
    return nbs_max_amt
...
def ftba_amt_calc(ftb_inc, ch_0012, ch_1315, ch_1619_sec, ch_00, ch_dep, ra_max_amt, ch_maint, maint_inc_p1, maint_inc_p2,
	...
	for i in range(obs.size):
        ...
        if ftb_inc[i] <= ftba_supp_inc_lmt:
            supp_amt[i] = ftba_supp * (ch_0012[i] + ch_1315[i] + ch_1619_sec[i])
		...
		if ftb_inc[i] >= ftba_free_area[1]:
			...
			m2_std_max[i] = ftba_std_amt_calc(ch_0012[i], ch_1315[i],
									          ch_1619_sec[i], ftba_std_rt,
											  use_max=False)
			# energy supplement at base
			m2_es_max[i] = ftba_std_amt_calc(ch_0012[i], ch_1315[i],
								             ch_1619_sec[i], ftba_es_rt,
											 use_max=False)
            ...
            m2_max[i] = (m2_std_max[i]
						 ...
						 + supp_amt[i])
			# income test reduction amount
			m2_inc_test_redn[i] = ftba_inc_test_calc(ftb_inc, ftba_tpr[1],
                                                     ftba_free_area[1])
